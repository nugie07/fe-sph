...
@section('scripts')
  ...
  <script>
    ...
    function showWorkflowModal(id) {
      ...
      $('#v_fob').val(row.fob || '');
      $('#v_term').val(row.term);
      // Helper function to format Indonesian currency string
      function formatIndonesianCurrency(value) {
        if (value && typeof value === 'string') {
          // Remove dots and convert to number
          var cleanValue = value.replace(/\./g, '');
          var val = parseFloat(cleanValue) || 0;
        } else {
          var val = value != null ? parseFloat(value) : 0;
        }
        return 'Rp ' + val.toLocaleString('id-ID', {minimumFractionDigits:2});
      }

      $('#v_transport').val(formatIndonesianCurrency(row.transport));
      $('#v_portal').val(formatIndonesianCurrency(row.portal));
      $('#v_harga').val(formatIndonesianCurrency(row.harga));
      $('#v_qty').val(row.qty || '');
      $('#v_sub_total').val(formatIndonesianCurrency(row.sub_total));
      $('#v_total').val(formatIndonesianCurrency(row.total));
      ...
      $('#form-verify-po').data('po-id', row.id);
        ...
        console.log('Transporter row data:', row); // Debug log
        console.log('Using trxId:', row.trxId || row.id); // Debug log
        loadRemarks(row.trxId || row.id, 'po_transporter', '#remarkTransporter');
    ...
    $(function(){
      ...
      function fetchSph(){
        ...
        $('#basic-1 tbody').html(
          ...
          .done(function(res){
            ...
            // Update card counts from new response structure
            $('#card-total_sph').text(res.data.sph.count || 0); // SPH Approval
            $('#card-waiting').text(res.data.supplier.count || 0); // PO Supplier Approval
            $('#card-revisi').text(res.data.transporter.count || 0); // PO Transporter Approval
            $('#card-invoice').text(res.data.invoice.count || 0); // Invoice Approval

            // Check if data exists and has the expected structure
            if (!res.data || !res.data.sph || !res.data.sph.items || !Array.isArray(res.data.sph.items)) {
              console.error('No data or invalid data structure:', res);
              table.clear().draw();
              return;
            }
            ...
            var rows = res.data.sph.items.map(function(item, index){
              ...
              return [
                ...
                item.produk_dibeli || '',
                formatRupiah(item.harga_per_liter || 0),
                formatRupiah(item.ppn || 0),
                formatRupiah(item.pbbkb || 0),
      ...
      function formatRupiah(x){
        // Handle string format like "1.200.200" (Indonesian format)
        if (x && typeof x === 'string') {
          // Remove dots and convert to number
          var cleanValue = x.replace(/\./g, '');
          x = parseFloat(cleanValue) || 0;
        } else {
          x = parseFloat(x) || 0;
        }
      ...
      var invoiceTable = $('#invoice-table').DataTable({
          ...
          columns: [
              ...
              },
              {
                  ...
                  orderable: false,
                  render: function(id) {
                      return '<span class="badge bg-primary verify-invoice-btn" data-id="' + id + '" style="cursor:pointer;">Click To Verify</span>';
                  }
              }
          ],
          drawCallback: function() {
      ...
      $(document).on('click', '.verify-supplier-btn', function() {
        ...
        $('#sp_drs_unique').val(row.id);
        $('#sp_category').val(row.category || 1);
        ...
        $('#sp_transport').val(formatIndonesianCurrency(row.transport));
        $('#sp_harga').val(formatIndonesianCurrency(row.harga));
        $('#sp_qty').val(row.qty || '');
        $('#sp_sub_total').val(formatIndonesianCurrency(row.sub_total));
        $('#sp_ppn').val(formatIndonesianCurrency(row.ppn));
        $('#sp_pph').val(formatIndonesianCurrency(row.pph));
        $('#sp_pbbkb').val(formatIndonesianCurrency(row.pbbkb));
        $('#sp_bph').val(formatIndonesianCurrency(row.bph));
        $('#sp_total').val(formatIndonesianCurrency(row.total));
        ...
        console.log('Supplier row data:', row); // Debug log
        console.log('Using trxId:', row.trxId || row.id); // Debug log
        loadRemarks(row.trxId || row.id, 'po_supplier', '#remarkSupplier');
        ...
        $.ajax({
          ...
          complete: function() {
            ...
            $btn.find('.txt').removeClass('d-none');
                  ...
                  // Invoice Approval Modal Event Handler
      $(document).on('click', '.verify-invoice-btn', function() {
        const invoiceId = $(this).data('id');
        console.log('Invoice verify button clicked, ID:', invoiceId);
        ...
        if (!finalInvoiceId) {
          // Try to get from text content or other attributes
          const buttonText = $(this).text();
          console.log('Button text:', buttonText);

          // Try to get from parent row data
          const row = $(this).closest('tr');
          if (row.length) {
            const rowData = invoiceTable.row(row).data();
            if (rowData && rowData.id) {
              finalInvoiceId = rowData.id;
              console.log('Got ID from row data:', finalInvoiceId);
            }
          }
        }
        ...
        console.log('Final Invoice ID:', finalInvoiceId);

        // Set invoice ID to submit button before showing modal
        $('#btn-submit-approval').data('invoice-id', finalInvoiceId);
