                    ...
                    <table class="table table-bordered" id="oat-table">
                      <thead>
                        <tr>
                          ...
                          <th style="width: 30%;">OAT 10 kl</th>
                          <th style="width: 25%;">SPOB</th>
...
<script>
    $(document).ready(function() {
        ...
        function addOatRow(lokasi = '', oat10kl = '', spob = '') {
            ...
            const row = `
                <tr id="${rowId}">
                    ...
                    <td>
                        <input type="text" class="form-control oat-spob" value="${spobValue}" placeholder="0">
                    </td>
        ...
        // Inisialisasi default lokasi (Kaliorang dan Kutai)
        function initializeDefaultOatRows() {
            $('#oat-table-body').empty();
            addOatRow('Kaliorang', '550', '650');
            addOatRow('Kutai', '250', '');
        }

        // Fungsi untuk mengumpulkan data OAT saat submit (format seperti kencana.blade.php)
        function getOatData() {
            const details = [];
            ...
            $('#oat-table-body tr').each(function() {
                ...
                const spob = $(this).find('.oat-spob').val() || '';
                
                if (lokasi) {
                    // Parse spob value, jika kosong set null
                    const spobValue = spob.trim() ? parseFloat(spob) || null : null;
                    
                    // Entri pertama: OAT 10 KL
                    details.push({
                        ...
                        spob: spobValue         // SPOB value (null jika kosong)
                    });
                    
                    // Entri kedua: SPOB
                    details.push({
                        biaya_lokasi: lokasi,  // Lokasi name
                        cname_lname: 'SPOB',  // SPOB
                        product: productText,    // Product name
                        qty: 0,                  // Qty (not used in OAT table, set to 0)
                        price_liter: oat10kl,   // OAT 10 kl value
                        ppn: 0,                  // PPN (calculated separately in main form)
                        pbbkb: 0,                // PBBKB (calculated separately in main form)
                        total_price: spobValue,  // SPOB value masuk ke total_price (null jika kosong)
                        spob: spobValue         // SPOB value (null jika kosong)
                    });
                }
            });
        ...
        $('#comp_name').on('change', function() {
            ...
            if (id) {
                ...
                $('#susut').val(''); $('#pbbkb_percentage').val('');
            }
        });
        ...
        $('#sph-form').on('submit', function(e) {
            ...
            // Build details array from OAT table (format seperti kencana.blade.php)
             ...
             *   template_id: "1",
             *   tipe_sph: "IASE",
             *   kode_sph: "02/BWS/IASE/I/P1/2024",
             *   comp_name: "PT BERKAT WAHANA SUKSES",
             *   pic: "John Doe",
             *   contact_no: "08123456789",
             *   email: "john@example.com",
             *   product: "Solar",
             *   price_liter: 10000,
             *   biaya_lokasi: "Kalimantan Timur (7.5%)",
             *   ppn: 1100,
             *   pbbkb: 750,
             *   total_price: 11850,
             *   pay_method: "Cash",
             *   susut: "0.1",
             *   note_berlaku: "Harga berlaku dari tanggal 1 - 14 Januari 2024",
             *   details: [
             *     {
             *       biaya_lokasi: "Kaliorang",
             *       cname_lname: "PT BERKAT WAHANA SUKSES",
             *       product: "Solar",
             *       qty: 0,
             *       price_liter: 550,
             *       ppn: 0,
             ...
             * 
             * Endpoint:
             * - Create: POST /api/sph/validator
             * - Update (Revisi): POST /api/sph/update
             */
            const formData = {
                ...
                details: details  // Kirim sebagai details array (format seperti kencana.blade.php)
            };
