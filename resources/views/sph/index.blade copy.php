      ...
      <div class="modal-body">
        <form id="create-sph-form" novalidate>
          ...
          <div class="mb-3">
            ...
            <select id="field-template" class="form-select" required>
              <option value="">Pilih Template</option>
            </select>
            <div class="invalid-feedback">Template wajib dipilih.</div>
          </div>
        </form>
      </div>
...
<script>
    ...
    if ($.fn.select2) {
        $('#field-tipe, #field-template').select2({
    ...
    $('#field-tipe').on('change', function(){
        ...
        if (type) {
            ...
            // Reload templates filtered by tipe
            loadSphTemplates(type);
        } else {
            $company.empty().append('<option value="">Pilih Customer</option>').trigger('change.select2');
            // Clear and disable template when tipe kosong
            loadSphTemplates(null);
        }
    });

    // Load SPH templates (filtered by tipe)
    function loadSphTemplates(type){
        var $tmpl = $('#field-template');
        $tmpl.prop('disabled', !type);
        if (!type) {
            $tmpl.html('<option value="">Pilih Tipe SPH terlebih dahulu</option>').trigger('change.select2');
            return;
        }
        $tmpl.html('<option value="">Loading...</option>').trigger('change.select2');
        $.get('/api/master-lov/sph_template', { type: type }, function(res){
            var list = res && res.data ? res.data : [];
            $tmpl.empty().append('<option value="">Pilih Template</option>');
            list.forEach(function(t){
                $tmpl.append('<option value="'+ t.id +'" data-form="'+ (t.form||'') +'">'+ (t.nama||('Template #'+t.id)) +'</option>');
            });
            $tmpl.trigger('change.select2');
        });
    }
    // Initial: disabled until tipe selected
    loadSphTemplates(null);
    ...
    $('#btn-modal-pilih').on('click', function(){
        ...
        $('#field-tipe, #field-company, #field-template').removeClass('is-invalid');
        if (!form.checkValidity()) {
            ...
            if (!$('#field-template').val()) $('#field-template').addClass('is-invalid');
            return;
        }
        var tipe = $('#field-tipe').val();
        var comp = $('#field-company').val();
        var tmplId = $('#field-template').val();
        var tmplForm = $('#field-template').find(':selected').data('form') || '';

        // If we have a form name, open the corresponding view inside a modal iframe
        if (tmplForm) {
            // Build a URL to dynamic form route
            var formUrl = '{{ route('sph.form.dynamic', ['form' => '___FORM___']) }}'
                ...
                + '&company_name=' + encodeURIComponent($('#field-company option:selected').text())
    ...
    $('#field-tipe, #field-company, #field-template').on('change', function(){
    ...
    $('#createSphModal').on('shown.bs.modal', function(){
        ...
        $('#field-tipe, #field-company, #field-template').removeClass('is-invalid').trigger('change.select2');
        loadSphTemplates(null);
    });
});
</script>
